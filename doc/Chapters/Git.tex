...
and to create a specific branches with a succinct label describing the purpose of the task or 
...
\section{Merging and resolving conflicts from VS}
 ...
 If conflicts appear click on \texttt{Resolve conflicts} and then look for Git changes 
 ...
 \item Select Gi/Manage branches. 
 \item Click right button on the branch to merge and click on merge this branch 
 into master branch. 
 \item Click on Resolve conflicts (in blue).
 \item Select Git changes to identify a list of files with conflicts. 
...
\end{enumerate}
If changes are not still committed, select \texttt{Git changes} a list of files that 
have been modified is shown. 
Click right button on the file we want to revert the change and select \texttt{Undo Changes}
to revert to the last change or select \texttt{View History} to recover some 
specific version of this file.  


If changes are already committed, select   \texttt{Git/View Branch History}
...
\item Click Revert (in blue). 
\item Select Git changes.  Click right button on the file and select {\bf merge}
...
\begin{enumerate}
\setlength\itemsep{0.0cm}
%\item Click on \textit{Issues}.
%\item Click on the green box \textit{New issue}.
