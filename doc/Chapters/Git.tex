...
Git is software  for tracking changes in computer files and making it ideal for software development (individual and among groups). 
Once it is installed in the computer any project can be managed by Git, that project is then called repository.
Once some project is tracked by Git, a \texttt{.git} hidden folder and some other files (i.e. the file \texttt{.gitignore}) appear in the 
repository.
...
It is web-based hosting service that allows to store files in the cloud a private or public repository. 
The next sections will cover the creation of local and remote repositories as well as 
procedures to work with collaborators or teams by developing software or modifying files. 


 
 \section{Create a remote  GitHub repository}
 First of all, a GitHub account must be created to store remote repositories. 
 Go to \texttt{github.com/join} and create a GitHub account associated to your email.
 ...
 with our credentials and click \texttt{+} button close to your photo to create a repository. 
...
Click on \texttt{GIT/Create Git Repository}, 
...
Browse through your  GitHub web page to see your new repository. 
 ...
 \item {\bf From GitHub Desktop. }
  Open \texttt{GitHub Desktop} and sign in with your GitHub account 
  (\texttt{File/Options/Accounts}).  
  Create a new repository (\texttt{File/New repository}) by selecting its name, 
  a \texttt{REAMDE.md} file and a \texttt{.gitignore} template. 
  Click on \texttt{Create repository} and then \texttt{Publish repository}.
  Browse through your GitHub web page to see your new repository.
  \end{enumerate} 
...
There are three different was to clone a remote repository 
to create a local repository. 
Go to any GitHub public repository and from the green button  \texttt{Code}
click on: 
 \begin{enumerate}
 \item {\bf  Open with GitHub Desktop.}
 This option opens the GitHubDesktop application and allows to select the  
 ...
 This option opens Visual Studio and allows to select the local path where this repository 
  will be stored. Click on \texttt{Clone} to create the local repository. 
...
Sometimes, auxiliary files or executable files do not need to be tracked because 
they generated automatically from sources files and  their changes depend only on the
source files. By the reason, those files are ignored by \texttt{Git} by specifying in the 
...
the master branch. Even if you the owner of the repository,
...
The following procedure should be carried out: 
 ...
 but it is always safer to increase the frequency of commits is you don't fell comfortable 
 with those changes.  
 ...
 uploaded to GitHub if some collaborator wanted to check it or to give you feedback. 

 \end{enumerate}
If collaborators are going to work  in some repository, the owner of the repository
could invite people to  collaborate  (\texttt{settings/manage access}).
Even if you are not invited to collaborate, you can create a fork of any repository 
and to propose later a pull request.  
 
\section{How to merge two branches}
Finally, when you decide your task is finished, merge your branch with the master 
branch. If you are working alone or 
if you are working only in one specific task at a time, 
you will never  encounter conflicts with trying to merge different branches.   
However, if two collaborators are modifying the same file, conflicts should be resolved. 
This process is shown graphically in figure \ref{fig:WorFlowGit}. 
\begin{enumerate} 
\setlength\itemsep{0.0cm}
\item Collaborators 1 and 2 create branches  1 and  2 
respectively based on the same master branch. 
\item Imagine that collaborator 2, 
finishes his task before collaborator 1 and merges his branch 2 with the master 
branch. He will not find conflicts because no one has merged with the master branch after 
he creates branch 2. 
\item Imagine that collaborator 1 fisnishes his branch and tries to merge. 
If both collaborators are working on the same files, 
collaborator 1 could find conflicts when trying to merge branch 1 with the master branch 
because at the time he tries to merge,  master branch is different from what 
it was based on. 
Hence, when working in teams possible conflicts could arise and should be resolved. 
\end{enumerate} 
...
\begin{enumerate}
\item {\bf Create a pull request}. From Visual Studio or from GitHub
create a pull request to pull changes of some specific branch to be merged with the master branch. 
From GitHub web page \texttt{create the pull request} and  \texttt{merge} accepted changes
...
conflicts are resolved. Once this is done, \texttt{Mark as resolved} to accept changes
by and \texttt{Commit merge}.
  

\item {\bf Merge branches from Visual Studio}.
 Select \texttt{Git/Manage branches} and from the master branch,  merge  
 by clicking on the specific branch with the right button and choose
 \texttt{merge branch into master}.
\end{enumerate} 

Once conflicts are resolved and some specific branch is merged with the master branch, it is advisable 
to delete the local and the remote specific branch which was used to work in this task. 
...
\section{How to revert specific commits} 
 ...
 In the following sections we will deal with conflicts or how to revert specific changes
 once they are committed and pushed.
...
\begin{enumerate}
    ...
    collaborators. 
    Remember that the longer you work on a branch without merging with the master one, the more probable it will be for conflicts to appear when merging.
...
project that you want to “checkpoint.” You can also add a helpful commit message to remind yourself or your teammates what work you did (e.g. 
...
``Modify''. Both ways install the tool for your computer, independently of using it with Visual Studio, a different IDE or the command prompt.

Once finished, by opening a command line you should be able to execute a git command, for example type: \texttt{git help git} on your command 
